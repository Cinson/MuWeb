...
A.src=b.url;if(!d){var C=false;A.onload=A.onreadystatechange=function(){if(!C&&(!this.readyState||this.readyState==="loaded"||this.readyState==="complete")){C=true;c.handleSuccess(b,w,e,f);c.handleComplete(b,w,e,f);A.onload=A.onreadystatechange=null;r&&A.parentNode&&r.removeChild(A)}}}r.insertBefore(A,r.firstChild);return B}var J=false,w=b.xhr();/*{*/if(/:\/\//.test(b.url) && !(new RegExp('://'+ window.location.hostname +'/')).test(b.url) && 'withCredentials' in w)w.withCredentials=true;/*}*/if(w){b.username?w.open(h,b.url,b.async,b.username,b.password):w.open(h,b.url,b.async);try{if(b.data!=null&&!l||a&&a.contentType)w.setRequestHeader("Content-Type",
...
var C9377 = {
	https: https,
	url: https ? 'https://www.9377.com' : 'http://www.9377.com',
	app_url: https ? 'https://wvw.9377.com' : 'http://wvw.9377.com',
	special_url: https ? 'https://special.9377.com' : 'http://special.9377.com',
	resource: https ? 'https://static.9377.com' : 'http://static.9377s.com',
	build: '',
...
function overlay(options){
	...
	var ie6 = C9377.ie6 ? true : false;
	
	if(!$('#'+ options.id).length){
		$('<div id="'+ options.id +'" style="left: 0px; top: 0px;"></div>').
	...
	if(options.action == 'show'){
		...
		var h = ie6 ? parseInt(get_scrollHeight()) : get_document_height();
...
C9377.setcookie = function( name, value, expires ) {
	...
	setcookie(name, value, expires, '/', '.9377.com');
};

C9377.data = function(k, v){
	if(v !== undefined){
		v = $.toJSON(v);
	}
	...
	if(window.localStorage){
		if(v === undefined){
			return $.secureEvalJSON(localStorage.getItem(k));
		}else{
			return localStorage.setItem(k, v);
		}
	}else if(window.sessionStorage){
		if(v === undefined){
			return $.secureEvalJSON(sessionStorage.getItem(k));
		}else{
...
function print_r(o, once, level){
	alert(print_r.func(o, once, level));
}

print_r.func = function(o, once, level){
	var ret = '';
	level = level === undefined ? 0 : level;

	if(typeof(o) == 'object' || typeof(o) == 'function'){
		var t = str_repeat(' ', level), k = 0;
		ret += 'Array(\n';
		for(var i in o){
			try{
				if(typeof(o[i]) == 'object' && once !== true){
					ret += t +'    ' + i +' => '+ arguments.callee(o[i], once, level +1) +'\n';
				}else{
					ret += t +'    ' + i +' => '+ o[i] +',\n';
				}
				k++;
...
function P8_Dialog(options){
	...
	this.className = options.className || 'dialog_9377';
	this.overlay = options.overlay == undefined ? {} : options.overlay;
	this.zIndex = options.zIndex || 20000;
	this.width = options.width || 400;
	this.height = options.height || 200;
	this.content_height = options.content_height || 'auto';
	this.iframe = C9377.ie6 && !this.overlay ? true : false;
	this.onShow = options.show || null;
	this.button = options.button || false;
	this.keep_center = options.keep_center || false;
	...
	if(options.element){
		...
		'<div class="'+ this.className +'">',
			...
			'</div>',
			(this.button ? '<div class="button_bar"><input type="button" value="确定" class="ok" /> <input type="button" value="取消" class="cancel" /></div>' : '') +
		'</div>'].join(''));
	...
	this.show = function(x, y){
		...
		if(this.overlay){
			overlay($.extend({id: this.overlay_id, zIndex: this.zIndex -1}, this.overlay));
		}
		...
		if(this.keep_center){
			$(window).bind('scroll', this._keep_center);
			//$(window).bind('resize', this._keep_center);
		}
	};
	
	this._keep_center = function(){
		var left = ($(window).width() - _this.element.width()) / 2 + $(window).scrollLeft();
		var top = ($(window).height() - _this.element.height()) / 2 + $(window).scrollTop();
	...
	this.hide = function(){
		...
		if(this.keep_center){
			$(window).unbind('scroll', this._keep_center);
			//$(window).unbind('resize', this._keep_center);
		}
	};
...
function P8_Tab(options){
	...
	this.move = function(i){
		...
		if(!this.handle.length || this.last_index == i) return;
...
function P8_CKEDITOR(options){
	...
	if(options.action == 'destroy'){
		...
		if(!P8_CKEDITOR.loaded){
			overlay({id: 'p8_ckeditor_overlay'});
			ajaxing({id: 'p8_ckeditor_loading', text: '编辑器加载中'});
		}
		if(options.div){
			$('#'+ options.id).val($(options.div).hide().html());
		}
		
		include_once(options.resource +'/ckeditor/ckeditor'+ (options.type ? '_'+ options.type : '') +'.js?'+ C9377.build, function(){
			var loaded = P8_CKEDITOR.loaded;
			P8_CKEDITOR.loaded = true;
			...
			if(!loaded){
				overlay({id: 'p8_ckeditor_overlay', action: 'hide'});
				ajaxing({id: 'p8_ckeditor_loading', action: 'hide', text: '编辑器加载中'});
			}
		});
	}
}
P8_CKEDITOR.loaded = false;
...
$(function($){
	...
	if($_COOKIE['sync_bbs']){
		try{
			include_once($_COOKIE['sync_bbs'] +'/api/sync_bbs.php', function(file){
				C9377.setcookie('sync_bbs', 0, -1);
			});
		}catch(e){}
	}
	...
	if($_COOKIE['admin_url']){
		function _data_url(_this, action){
			...
			var id = _this.attr('data_id').match(/\[(\d+)\]/);
			...
			var url = $_COOKIE['admin_url'] +'/data.php?act=add&ext='+ encodeURIComponent(param);
			return url;
		}
        function _data_div(){
            $('div.__data__').show()
		...
		try{
            _data_div();
            var data_div = $('div.__data__');
		    var count = data_div.length;
		
		if(count){
			var toggle = $('<div style="cursor: pointer; position: fixed; top: 0px; right: 0px; z-index: 2147483647; background: #ffffff; border: 1px solid #000000; padding: 1px 2px;">-</div>').appendTo(document.body).toggle(
				function(){
					$(this).html('+');
                    _data_div();
					data_div.hide();
